    ...
    <div class="d-flex justify-content-between">
        ...
        <h4 class="fw-bold py-1 mb-3">
           <a href="#newModal" class="btn btn-md btn-primary waves-effect pull-right" data-bs-toggle="modal">
            <i class="fas fa-plus"></i> &nbsp; Create Kategori item
            </a>
        </h4>
    </div>
                    ...
                    <menu id="nestable-menu">
                      ...
                      <div class="col-md-8">
                        <p id="success-indicator" style="display:none; margin-right: 10px;">
                            <i class="fas fa-check-circle"></i> Kategori order has been saved
                        </p>
                    ...
                    <div class="modal-dialog modal-dialog-scrollable modal-md" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title"><span id="title_form">Create</span> Kategori item</h5>
                            <button
                            type="button"
                            ...
                            <div class="modal-body" style="background:#f7f7f9;">
                                ...
                                <form id="form_kategori" action="" autocomplete="off">

                                    <div class="form-floating form-floating-outline mb-3 mt-3">
                                        <select class="select2 form-select" 
                                            id="main_kategori_berita" name="main_kategori_berita">
                                            <option value="">-- Main Menu --</option>                                                  
                                            @foreach ($mainkategori as $data)
                                        ...
                                        <label for="main_kategori_berita">Main Kategori</label>
                                    </div>

                                    <div class="form-floating form-floating-outline mb-3 mt-3">
                                        <input type="text" class="form-control" id="nama_kategori_berita" name="nama_kategori_berita" 
                                        placeholder=" " aria-describedby="floatingKategoriName">
                                        <label for="Nama Kategori">Nama Kategori</label>
                                        <div id="floatingKategoriName" class="form-text">
                                        </div>
                                    </div>


                                    <div class="form-floating form-floating-outline mb-3 mt-3" hidden>
                                        <input type="text" class="form-control" id="kategori_urut" name="kategori_urut" 
                                        placeholder=" " aria-describedby="floatingKategoriUrut">
                                        <label for="Kategori Urut">Kategori Urut</label>
                                        <div id="floatingKategoriUrut" class="form-text">
                                        </div>
                                    </div>

                                    <div class="form-floating form-floating-outline mb-3 mt-3" hidden>
                                        <input type="text" class="form-control" id="status_kategori" 
                                        name="status_kategori" placeholder=" " 
                                        aria-describedby="floatingKategoriStatus">
                                        <label for="Status">Status</label>
                                        <div id="floatingKategoriStatus" class="form-text">
                                        </div>
                                    </div>

                                    <div class="form-floating form-floating-outline mb-3 mt-3" hidden>
                                        <input type="text" class="form-control" id="id_kategori_berita" name="id_kategori_berita" 
                                        placeholder=" " aria-describedby="floatingIDKategori">
                                        <label for="ID Kategori">ID Kategori</label>
                                        <div id="floatingIDKategori" class="form-text">
                                        </div>
                                    </div>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit" class="btn btn-primary" id="add-kategori"><span id="aksi_submit">Create</span></button>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
    $(document).ready(function(){
        ...
        nestableKategori();
        //load nestable2
        function nestableKategori(){
            $('#nestable').html('');
               $.ajax({  
               url:`{{ route("nestableKategori") }}`,
               method:'get',  
               data: { 
               ...
               dataType : "JSON",  
                   ...
                   if (data) {  
                            setTimeout(function(){// wait for 5 secs(2)
                                $('#nestable').html(data.kategori); // then reload the page.(3)
                            }, 100); 
        ...
        function clearForm(){
            ...
            $('#main_kategori_berita').trigger('change');
            $("#aksi_submit").text("Create");
            $("#title_form").text("Create");
        }

        $(document).on('change', '#main_kategori_berita', function () {
            var id = $(".select2 option:selected").val();
            var status_form = $('#status_kategori').val();
                if(status_form==''){
                    getLastUrutKat(id);
                }
            ...
            $(document).on('click', '.editKategori', function () {
                ...
                dataType : "JSON",  
                    ...
                    if (data.status == "success") {  
                            $('#id_kategori_berita').val(data.hasil.id_kategori_berita);
                            $('#nama_kategori_berita').val(data.hasil.nama_kategori_berita);
                            $('#kategori_urut').val(data.hasil.urut);
                            $('#status_kategori').val('edit');
                            $('#main_kategori_berita').val(data.hasil.main_kategori_berita);
                            $('#main_kategori_berita').trigger('change');
                        } 
        ...
        function getLastUrutKat(id){
               
                $.ajax({  
                url:`{{ route("getLastUrutKat") }}`,
                method:'POST',  
                data: { 
                    _token: $('meta[name="csrf-token"]').attr('content'),
                    id : id} ,  
            ...
            function save_new_nav(){
                var formdata = $('#form_kategori').serializeIncludeDisabled();
            ...
            function save_edit_nav(){
                ...
                url:`{{ route("kategori.storeUpdate") }}`,
                method:'POST',  
                data: formdata,  
                dataType : "JSON",  
                    success:function(data)  
